...
/obj/machinery/atmospherics/unary/vent_pump/attackby(obj/item/W, mob/user)
	...
	if(istype(W, /obj/item/tool/pickaxe/plasmacutter))
		...
		if(!welded)
			...
			return
		else if(P.cell.charge >= P.charge_cost * vlow_mod && P.powered)
			P.start_cut(user, src.name, src)
			if(do_after(user, P.calc_delay(user) * vlow_mod, TRUE, 5, BUSY_ICON_HOSTILE) && P)
				P.cut_apart(user, src.name, src, P.charge_cost * vlow_mod) //Vents require much less charge
				welded = FALSE
				update_icon()
			return
		else
