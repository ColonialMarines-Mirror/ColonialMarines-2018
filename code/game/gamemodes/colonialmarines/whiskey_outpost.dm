...
/obj/item/device/laz_designator/proc/laz(var/mob/living/carbon/human/user, var/atom/A, var/params)
	...
	message_admins("ALERT: [user] ([user.key]) IS CURRENTLY LAZING A TARGET: CURRENT MODE [laz_mode], at ([T.x],[T.y],[T.z]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[T.x];Y=[T.y];Z=[T.z]'>JMP</a>) (<A HREF='?_src_=holder;adminplayerfollow=\ref[user.name]'>FLW</a>).") // Alert all the admins to this asshole. Added the jmp command from the explosion code.
