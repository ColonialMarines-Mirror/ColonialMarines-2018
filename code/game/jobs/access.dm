...
/proc/get_centcom_access_desc(A)
	switch(A)
		if(ACCESS_WY_PMC_GREEN)			return "NT PMC Green"
		if(ACCESS_WY_PMC_ORANGE)		return "NT PMC Orange"
		if(ACCESS_WY_PMC_RED)			return "NT PMC Red"
		if(ACCESS_WY_PMC_BLACK)			return "NT PMC Black"
		if(ACCESS_WY_PMC_WHITE)			return "NT PMC White"
		if(ACCESS_WY_CORPORATE)			return "NT Executive"
		if(ACCESS_IFF_PMC) 				return "NT Identification"
