	...
	if(istype(item, /obj/item/tank))
		...
		if(!tank_one)
			...
			if(user.drop_held_item())
				...
				item.forceMove(src)
				to_chat(user, "<span class='notice'>You attach the tank to the transfer valve.</span>")
				message_admins("[key_name_admin(user)] attached both tanks to a transfer valve. (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[location.x];Y=[location.y];Z=[location.z]'>JMP</a>) (<A HREF='?_src_=holder;adminplayerfollow=\ref[user.name]'>FLW</a>)")
...
//TODO: Have this take an assemblyholder
		...
		message_admins("[key_name_admin(user)] attached a [item] to a transfer valve. (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[location.x];Y=[location.y];Z=[location.z]'>JMP</a>) (<A HREF='?_src_=holder;adminplayerfollow=\ref[user.name]'>FLW</a>)")
