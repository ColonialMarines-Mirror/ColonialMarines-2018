...
/obj/item/stack/sheet/glass/proc/construct_window(mob/user)
	...
	if(!user.IsAdvancedToolUser())
		...
		return 0
	if(ishuman(user) && user.mind && user.mind.cm_skills && user.mind.cm_skills.construction < SKILL_CONSTRUCTION_PLASTEEL)		user.visible_message("<span class='notice'>[user] fumbles around figuring out how to build with [src].</span>",
