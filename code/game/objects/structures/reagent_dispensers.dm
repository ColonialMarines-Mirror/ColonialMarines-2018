...
/obj/structure/reagent_dispensers/fueltank/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (istype(W,/obj/item/tool/wrench))
		...
		modded = modded ? 0 : 1
		if (modded)
			message_admins("[key_name_admin(user)] opened fueltank at [loc.loc.name] ([loc.x],[loc.y],[loc.z]), leaking fuel. (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[loc.x];Y=[loc.y];Z=[loc.z]'>JMP</a>) (<A HREF='?_src_=holder;adminplayerfollow=\ref[user]'>FLW</a>)")
	...
	if (istype(W,/obj/item/device/assembly_holder))
		...
		if(do_after(user, 20, TRUE, 5, BUSY_ICON_HOSTILE))
			...
			if (istype(H.a_left,/obj/item/device/assembly/igniter) || istype(H.a_right,/obj/item/device/assembly/igniter))
				message_admins("[key_name_admin(user)] rigged fueltank at [loc.loc.name] ([loc.x],[loc.y],[loc.z]) for explosion. (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[loc.x];Y=[loc.y];Z=[loc.z]'>JMP</a>) (<A HREF='?_src_=holder;adminplayerfollow=\ref[user]'>FLW</a>)")
...
/obj/structure/reagent_dispensers/fueltank/bullet_act(var/obj/item/projectile/Proj)
	...
	if(istype(Proj.firer,/mob/living/carbon/human))
		message_admins("[key_name_admin(Proj.firer)] shot fueltank at [loc.loc.name] ([loc.x],[loc.y],[loc.z]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[loc.x];Y=[loc.y];Z=[loc.z]'>JMP</a>) (<A HREF='?_src_=holder;adminplayerfollow=\ref[Proj.firer]'>FLW</a>).")
