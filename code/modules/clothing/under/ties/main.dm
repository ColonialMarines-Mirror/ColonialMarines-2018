/obj/item/clothing/tie
	...
	w_class = 2.0
	var/obj/item/clothing/under/has_suit = null		//The suit the tie is attached to.
...
/obj/item/clothing/tie/stethoscope/attack(mob/living/carbon/human/H, mob/living/user)
	if(istype(H) && istype(user) && user.a_intent == "help")
		...
		if(body_part)
			...
			user.visible_message("[user] places [src] against [H]'s [body_part] and listens attentively.", "You place [src] against [H.gender == MALE ? "his" : "her"] [body_part]. You [sound_strength] [sound].")
			return
