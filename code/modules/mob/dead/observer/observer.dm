...
/mob/living/verb/ghost()
	...
	if(stat == DEAD)
		...
		if(location) //to avoid runtime when a mob ends up in nullspace
			message_admins("[key_name_admin(usr)] has ghosted. (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[location.x];Y=[location.y];Z=[location.z]'>JMP</a>) (<A HREF='?_src_=holder;adminplayerfollow=\ref[usr]'>FLW</a>)")
