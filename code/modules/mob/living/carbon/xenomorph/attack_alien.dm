...
/mob/living/carbon/human/attack_alien(mob/living/carbon/Xenomorph/M, dam_bonus, random_location = FALSE)
	if (M.fortify)
		return FALSE
	...
	switch(M.a_intent)
		...
		if("hurt")
			...
			if(!affecting || random_location) //No organ, just get a random one
