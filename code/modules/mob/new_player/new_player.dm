...
//	if(QDELETED(src) || !src.client) // no qdels (yet)
...
//		return FALSE
	...
	close_spawn_windows()
	//var/obj/effect/landmark/observer_start/O = locate(/obj/effect/landmark/observer_start) in GLOB.landmarks_list >landmarks not listed
	var/obj/O = locate("landmark*Observer-Start")
	src << "<span class='notice'>Now teleporting.</span>"
	if(O)
		observer.forceMove(O.loc) //no forceMove
	else
		src << "<span class='notice'>Teleporting failed. Ahelp an admin please</span>"
		//stack_trace("There's no freaking observer landmark available on this map or you're making observers before the map is initialised")
	observer.key = key
	observer.client = client
//	observer.set_ghost_appearance() //sad we dont have that
