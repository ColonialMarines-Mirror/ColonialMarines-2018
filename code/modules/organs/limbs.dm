...
/datum/limb/proc/take_damage(brute, burn, sharp, edge, used_weapon = null, list/forbidden_limbs = list(), no_limb_loss)
	...
	if(body_part != UPPER_TORSO && body_part != LOWER_TORSO && !no_limb_loss)
		...
		if(body_part != HEAD || ( body_part == HEAD && ( !istype(worn_helmet, /obj/item/clothing/head/helmet) || (owner.species.flags & IS_SYNTHETIC) ) ) ) //Pass through if limb isn't a head or if it IS but we're not wearing a helmet, or we're a synthetic
