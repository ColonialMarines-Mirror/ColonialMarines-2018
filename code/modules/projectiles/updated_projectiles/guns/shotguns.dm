...
/obj/item/weapon/gun/shotgun/combat/set_gun_config_values()
	...
	damage_mult = config.base_hit_damage_mult - (config.med_hit_damage_mult + config.min_hit_damage_mult)  //normalizing gun for vendors; damage reduced by 25% to compensate for faster fire rate; still higher DPS than M37.
	recoil = config.low_recoil_value
	recoil_unwielded = config.high_recoil_value
