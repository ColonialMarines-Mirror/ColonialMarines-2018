			...
			for(var/datum/internal_organ/I in H.internal_organs)
				if(I)
					if(I.robotic != ORGAN_ROBOT)
						if(I.damage > 0)
							if(M.bodytemperature < 170 || (I.damage <= I.min_bruised_damage * 0.5 && I != "eyes" && I != "brain")) // Peridaxon slowly heals organ damage if patient at cryo temp, or if damage minor.
...
/datum/reagent/quickclot
	...
	/datum/reagent/quickclot/on_mob_life(mob/living/M)
		. = ..()
		if(!.)
			return
