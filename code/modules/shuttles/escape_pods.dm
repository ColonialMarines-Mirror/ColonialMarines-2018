...
/datum/shuttle/ferry/marine/evacuation_pod/proc/prepare_for_launch()
	...
	if(!check_passengers())
		...
		spawn()
			...
			D.open()
			D.lock()
		evacuation_program.master.state("<span class='warning'>WARNING: Maximum weight limit reached, pod unable to launch.</span> <span class='danger'>DANGER: thruster failure Imminent.</span>")
		sleep(35)
